...
touch ./client.conf
sed -e "s~$(echo 'private_key')~${application_private_key}~g" -e "s~$(echo 'server_public_key')~${gateway_public_key}~g" -e "s~$(echo 'vpn_address')~${vpn_address}~g" -e "s~$(echo 'server_public_ip')~${gateway_public_ip}"  ./client-template.conf > ./client.conf
